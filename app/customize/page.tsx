...
export default function CustomizePage() {
  ...
  return <main className="config-page">
    ...
    <section id="configurator" className="config-shell shell">
      ...
      <div className="config-builder">
        ...
        <section id="material" className="config-block"><div className="block-head"><span>02</span><div><small>PERFORMANCE + PROFILE</small><h2>Choose your material</h2></div></div><div className="expert-nudge"><span>EXPERT MODE</span><p>Not sure which material fits the slope, attic, storm exposure, and ownership plan?</p><a href="/roof-advisor">ASK THE ROOF ADVISOR →</a></div><div className="material-options">{materials.map((m)=><button key={m.id} className={material===m.id?'selected':''} onClick={()=>setMaterial(m.id)}><span className={`material-swatch ${m.id}`}/><small>{m.life} PLANNING SERVICE RANGE</small><b>{m.name}</b><i>${m.rate.toFixed(2)} / SQ FT PLANNING BASE</i></button>)}</div></section>
