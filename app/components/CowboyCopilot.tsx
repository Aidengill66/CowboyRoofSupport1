...
function answer(input: string): Reply {
  ...
  if (q.includes('storm')) return { text: 'Start with safe ground-level photos, note interior moisture, and request a roof inspection. We document conditions; the insurer decides coverage.', href: '/services#inspection', label: 'OPEN STORM INSPECTION' };
  if (q.includes('metal') || q.includes('longest')) return { text: 'Standing seam metal is the long-life path when slope, deck, clips, underlayment, flashing compatibility, and ventilation are designed together. It costs more upfront than shingles.', href: '/roof-advisor', label: 'COMPARE METAL SYSTEM' };
  if (q.includes('shingle') || q.includes('value') || q.includes('material')) return { text: 'For many North Atlanta homes, a Class 4 architectural shingle is the strongest value starting point. Designer shingles favor curb appeal; metal favors long ownership.', href: '/customize', label: 'COMPARE MATERIALS' };
  ...
  if (q.includes('upgrade') || q.includes('performance')) return { text: 'Prioritize correct flashing and water barriers first, then balanced ventilation and drainage. Premium finishes matter only after the hidden system is right.', href: '/marketplace', label: 'VIEW PERFORMANCE UPGRADES' };
  ...
  if (q.includes('commercial') || q.includes('large')) return { text: 'Large projects begin with access, safety, staging, drainage, occupied-space protection, and schedule coordination—not a residential price formula.', href: '/services#commercial', label: 'VIEW LARGE-SCALE WORK' };
  ...
  if (q.includes('hat') || q.includes('boot') || q.includes('gear')) return { text: 'The Marketplace separates field goods from roof systems, so you can build a gear list without mixing merchandise into a roofing estimate.', href: '/marketplace', label: 'SHOP FIELD GOODS' };
  if (q.includes('layer')) return { text: 'A complete roof is deck, fastening, underlayment, water barriers, flashing, ventilation, drainage, and finish material. The visible shingle is only the top layer.', href: '/guides', label: 'OPEN BLUEPRINT GUIDE' };
  if (q.includes('inspect') || q.includes('book')) return { text: 'A free inspection is the cleanest starting point when the roof condition is uncertain. It should cover surface, flashing, penetrations, attic signals, ventilation, and drainage.', href: '/services#inspection', label: 'SEE THE INSPECTION' };
