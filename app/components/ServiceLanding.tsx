...
export function ServiceLanding({ eyebrow, title, intro, signal, problems, steps, faqs, afterHero }: ServiceLandingProps) {
  return <main className="service-landing">
    <section className="service-landing-hero"><div className="shell"><div><p className="eyebrow light">{eyebrow}</p><h1>{title}</h1><p>{intro}</p><div className="actions"><a className="primary signal-button" href="/free-inspection">GET A FREE INSPECTION <span>→</span></a><a className="service-call" href="tel:+14708342519">CALL (470) 834-2519</a></div></div><aside><small>CREW SIGNAL</small><strong>{signal}</strong><span><i/> NORTH ATLANTA ROUTING</span><span><i/> PHOTO-DOCUMENTED SCOPE</span><span><i/> HUMAN CONFIRMATION</span></aside></div></section>
    {afterHero}
    ...
    <section className="final-cta"><p className="eyebrow">THE NEXT STEP IS SIMPLE</p><h2>Put eyes on the roof.</h2><a className="primary" href="/free-inspection">REQUEST MY FREE INSPECTION <span>→</span></a></section>
  </main>;
}
